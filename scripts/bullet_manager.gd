...
func fire_bullet(dir: Vector3, speed: float = 10.0, lifetime: float = 5.0) -> void:
...
func pattern_radial_ring(count: int = 24, speed: float = 10.0, lifetime: float = 5.0) -> void:
	for i in range(count):
		var angle := (TAU * float(i)) / float(count)
		var dir := Vector3(cos(angle), 0.0, sin(angle))
		fire_bullet(dir, speed, lifetime)

func pattern_targeted_shot(target: Node3D, spread: float = 0.25, count: int = 5, speed: float = 10.0, lifetime: float = 5.0) -> void:
	if target == null:
		return
...
func pattern_spiral(turns: int = 40, step: float = 0.25, speed: float = 10.0, lifetime: float = 5.0) -> void:
	await _spiral_coroutine(turns, step, speed, lifetime)

func _spiral_coroutine(turns: int, step: float, speed: float, lifetime: float) -> void:
